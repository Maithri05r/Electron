...
//   // Listen on all IPv4 interfaces → reachable via LAN IPs
//   server.listen(TCP_PORT, "0.0.0.0", () => {
//     const ips = Object.values(os.networkInterfaces())
//       .flatMap(a => a ?? [])
//       .filter(n => n?.family === "IPv4" && !n.internal)
//       .map(n => n!.address);
//     console.log(`🚀 TCP server running on 0.0.0.0:${TCP_PORT}`);
...
//   const { onMessage } = startTCPServer();
//   onMessage((msg, ip) => {
//     console.log(`💬 Received from ${ip}: ${msg}`);
//   });
// }
